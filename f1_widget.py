import requests
import json
import traceback # ÚJ: Hiba nyomkövetéshez
...
def main():
    ...
    try:
        ...
        if not next_data:
            print("HIBA: Nem sikerült letölteni a next.json-t")
        ...
        # --- IDŐPONT KEZELÉS JAVÍTÁSA (UTC) ---
        now = datetime.now(timezone.utc)
        
        sessions = []
        def add_session(name, date_str, time_str, duration_min):
            # A 'Z' betű eltávolítása a biztonság kedvéért, majd explicit UTC beállítás
            clean_time = time_str.replace("Z", "")
            # FORCE UTC: Így biztosan nem akad össze a 'now'-val
        ...
        # Schedule Generálás (Időpontokkal!)
        schedule_text = ""
        for s in sessions:
            local_time = s["start"].astimezone() # Átváltás helyi időre
        ...
        # Ez a rész kiírja a hibát a logba, hogy lássuk mi a baj
        print("VÉGZETES HIBA TÖRTÉNT:")
        traceback.print_exc()

    # Mentés akkor is, ha hiba volt (a régi adatokkal vagy az alapértelmezett placeholderrel)
    with open(JSON_OUTPUT_PATH, 'w') as f: json.dump(widget_data, f)
    print("Mentés kész.")
...
def main():
    ...
    try:
        ...
        # --- JAVÍTOTT MENETREND GENERÁLÁS (Dátum + Idő) ---
        schedule_text = ""
        for s in sessions:
