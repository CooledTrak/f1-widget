...
# Fix dátumok (Ezeket évente frissíteni kell)
...
# TECHNIKAI ADATOK: Körszám | Pályahossz | Körrekord
# (A 2025-ös szezon adatai alapján)
TRACK_SPECS = {
...
def main():
    widget_data = {
        ...
        "is_live": 0,
        "podium_title": "",
        # Pilóta adatok (Kód + Pont)
        "d1_c": "", "d1_p": "", 
        "d2_c": "", "d2_p": "", 
        "d3_c": "", "d3_p": "", 
        # Konstruktőri adatok (Kód + Pont)
        "c1_c": "", "c1_p": "", 
        "c2_c": "", "c2_p": "", 
        "c3_c": "", "c3_p": ""
    }
    ...
    if not next_data:
        # Ha nincs válasz, menti az üreset
    ...
    try:
        ...
        else: widget_data['track_info'] = "ADATOK BETÖLTÉSE..."
        ...
        # Ha a versenyhétvége van: Verseny időjárás
        if first_session.date() <= now.date() <= last_session.date():
            txt, ico = get_weather_data(race['Circuit']['Location']['lat'], race['Circuit']['Location']['long'])
            widget_data['weather_text'], widget_data['weather_icon'] = txt, ico
        # Egyébként üres (a widget a helyit használja)
        else:
            widget_data['weather_text'], widget_data['weather_icon'] = "", ""
        ...
        if widget_data['is_live'] == 0:
            if is_weekend_mode: 
                ...
                days = (first_session - now).days
                if days <= 0: widget_data['status_text'] = "HAMAROSAN KEZDŐDIK"
                else: widget_data['status_text'] = f"{days} NAP VAN HÁTRA"

        # --- OKOS MENETREND GENERÁLÁS ---
        schedule_text = ""
        for s in sessions:
        ...
        try:
            ...
            widget_data['c1_c'] = c_res[0]['Constructor']['name'][:3].upper() # Pl. MCL
            widget_data['c1_p'] = c_res[0]['points']
            widget_data['c2_c'] = c_res[1]['Constructor']['name'][:3].upper()
            widget_data['c2_p'] = c_res[1]['points']
            widget_data['c3_c'] = c_res[2]['Constructor']['name'][:3].upper()
            widget_data['c3_p'] = c_res[2]['points']
        except: pass

        # 2. PILÓTÁK
        # Alapból bajnokságot mutatja (kivéve ha akarunk Pole-t, de egyszerűbb így a váltáshoz)
        ...
        try:
            ...
            widget_data['d1_c'] = d_res[0]['Driver']['code']
            widget_data['d1_p'] = d_res[0]['points']
            widget_data['d2_c'] = d_res[1]['Driver']['code']
            widget_data['d2_p'] = d_res[1]['points']
            widget_data['d3_c'] = d_res[2]['Driver']['code']
            widget_data['d3_p'] = d_res[2]['points']
        except: pass

        # --- PROGRESS BAR ---
        last_race_end_time = LAST_SEASON_END
        try:
            lr_data = get_json("https://api.jolpi.ca/ergast/f1/current/last.json")
            if lr_data:
                lr = lr_data['MRData']['RaceTable']['Races'][0]
